...
const sql = `
-- Atualizar a função para também sincronizar com a estruturação
...
BEGIN
    ...
    WHERE id = p_id
    RETURNING id, cnpj, nome_entidade, status, observacoes, data_analise, analisado_por, operacao_id
    INTO v_verificacao;
    
    -- Sincronizar com a estruturação (atualizar o status lá também)
...
async function run() {
  try {
    ...
    console.log('✅ Função atualizada com sincronização para estruturação!');
    await client.end();
  } catch (err) {

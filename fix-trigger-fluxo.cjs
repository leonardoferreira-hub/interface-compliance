...
const sql = `
-- Modificar o trigger para só sincronizar quando status for 'aceita'
...
CREATE TRIGGER trigger_sync_emissao_to_estruturacao
    ...
    EXECUTE FUNCTION sync_emissao_to_estruturacao_and_compliance();

-- Verificar função sync_emissao_to_estruturacao_and_compliance
...
BEGIN
    ...
    -- Só cria verificação no compliance se ainda não existir
...
async function run() {
  try {
    ...
    console.log('✅ Trigger corrigido! Agora só sincroniza quando status = aceita');
    await client.end();
  } catch (err) {

...
const sql = `
-- Criar funções RPC para operações de escrita no Compliance
...
DROP VIEW IF EXISTS public.v_compliance_dashboard;
DROP VIEW IF EXISTS public.v_compliance_verificacoes;
DROP VIEW IF EXISTS public.v_compliance_investidores;
DROP VIEW IF EXISTS public.v_compliance_cnpjs;

CREATE OR REPLACE VIEW public.v_compliance_dashboard AS 
SELECT * FROM compliance.v_dashboard;

CREATE OR REPLACE VIEW public.v_compliance_verificacoes AS 
SELECT * FROM compliance.verificacoes_pendentes;

CREATE OR REPLACE VIEW public.v_compliance_investidores AS 
SELECT * FROM compliance.investidores;

CREATE OR REPLACE VIEW public.v_compliance_cnpjs AS 
SELECT * FROM compliance.cnpjs_verificados;

-- Grants nas views
GRANT SELECT ON public.v_compliance_dashboard TO anon, authenticated;
GRANT SELECT ON public.v_compliance_verificacoes TO anon, authenticated;
GRANT SELECT ON public.v_compliance_investidores TO anon, authenticated;
GRANT SELECT ON public.v_compliance_cnpjs TO anon, authenticated;
...
async function runSQL() {
  try {
    ...
    console.log('   - Views públicas criadas');
